...
begin
     ...
     Result:=VarArrayCreate([0,stream.Size - 1],VarByte); //dbs
     try
...
{$ENDIF}
                              ...
                              else raise EMemTableInvalidRecord.Create('erro02');//kbmInvalidRecord);
                         end;
{$IFDEF DOTNET}
...
// i:integer;
begin
     pfdStart:=IntPtr(Integer(IntPtr(FilterExpr.FilterData))+8);
     pfd:=pfdStart;
