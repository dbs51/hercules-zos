...

{
      ...
      Notes:
       ...
       - you MUST set DestSize before call (inmemory decompression)

      initial release (c)2002 by Daniel Mantione
      modifications by Arnaud Bouchez (c)2008 http://bouchez.info
...
procedure hb_create_decode_tables(var limit,base,perm:array of cardinal;
  var length:array of byte; minlen,maxlen:byte;alphasize:cardinal);
...
begin
  ...
  {Read the magic.}
  if (readstream[0]<>'B') or (readstream[1]<>'Z') or (readstream[2]<>'h') then begin
      error := BZip2_bad_header_magic;
      exit;
    end else inc(readstream,3);
...
// while get_boolean do begin
      repeat
        if bits_available<=0 then begin
          ...
          read_data:=byte(data shl 1);
          bits_available := 7;
          if data shr 7=0 then break;
          inc(j); if j<=5 then continue else begin error :=BZip2_data_error; break; end;
        end
        else begin
...
//  zvec:=get_bits(zn);
  if zn>bits_available then begin
      ...
      read_data := byte (data shl (zn-bits_available));
      inc(bits_available,n8);
    end
    else begin
      zvec := read_data shr (8-zn);
      read_data:= byte(read_data shl zn);
...
rle_write:
        if rle_len=1 then begin
          ...
          rle_len := 0;
          inc(bufptr);
          if count<>0 then continue else break;
        end else
